...
async function saveData() {
  ...
  await supabaseClient.from('content_data')
    .delete()
  ...
  for (const platform in contentData) {
    for (const month in contentData[platform]) {
      await supabaseClient.from('content_data').insert({
...
function addContentRowWithData(data = {}, index) {
    ...
    tr.innerHTML = `
  <td><input type="text" value="${data.judul || ''}"oninput="saveContent(); debounceSave();"></td>

  <td>
    <select oninput="saveContent(); debounceSave();">
  ...
  <!-- REFERENSI -->
  <td>
    <input type="url" value="${data.referensi || ''}" oninput="saveContent(); debounceSave();" placeholder="https://...">
    ${data.referensi ? `<br><a href="${data.referensi}" target="_blank">Buka</a>` : ''}
  </td>
  ...
  <td><input type="text" value="${data.produk || ''}" "oninput="saveContent(); debounceSave();"></td>
  ...
  <!-- UPLOAD -->
  <td>
    <input type="url" value="${data.linkUpload || ''}" oninput="saveContent(); debounceSave();" placeholder="https://...">
    ${data.linkUpload ? `<br><a href="${data.linkUpload}" target="_blank">Buka</a>` : ''}
  </td>

  <!-- TOTAL VIEWS (MANUAL) -->
  <td>
    <input type="number" value="${data.views || 0}" min="0" oninput="saveContent(); debounceSave();">
  </td>
...
async function loadFromSupabase() {
    const { data, error } = await supabaseClient.from('content_data')
        .select('*')
        .eq('user_id', currentUser.id);
...
supabaseClient.channel('realtime-content')
  .on(
    'postgres_changes',
